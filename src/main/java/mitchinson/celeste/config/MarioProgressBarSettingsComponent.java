package mitchinson.celeste.config; // Copyright 2000-2022 JetBrains s.r.o. and other contributors. Use of this source code is governed by the Apache 2.0 license that can be found in the LICENSE file.

import static mitchinson.celeste.MBCharacter.*;
...
public class MarioProgressBarSettingsComponent {
    ...
    private final List<JBRadioButton> charactersRadioButtons =
        new ArrayList<>();
    ...
    public MarioProgressBarSettingsComponent() {
        ...
        FormBuilder formBuilder = FormBuilder.createFormBuilder().addComponent(
            title
        );
    ...
    public void setChosenCharacter(@NotNull MBCharacter newBros) {
        for (JBRadioButton charactersRadioButton : charactersRadioButtons) {
            charactersRadioButton.setSelected(
                newBros
                    .name()
                    .equals(charactersRadioButton.getText().toUpperCase())
            );
        }
    }
}
